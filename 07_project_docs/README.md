...
### Top-40 Dataset Selection Strategy
...
2. Stratify across five condition dimensions:
   - **Size**: small (<1k rows), medium (1k-20k), large (>20k)
   - **Dimensionality**: low (≤10), mid (11-100), high (>100)
   - **Feature type**: numeric, categorical, mixed
   - **Class imbalance**: balanced (<0.05), moderate (0.05-0.2), high (>0.2)
   - **Classes**: binary (2), multiclass-few (3-4), multiclass-many (5+)
...
5. Result: **40 datasets** covering condition space
...
### Output Files
- `results/final_40_datasets_for_mlp_condition_based.csv`: Metadata for all 40 selected datasets + condition assignments
